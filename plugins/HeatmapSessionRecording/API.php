...
class API extends \Piwik\Plugin\API
{
     ...
     * @return int
     */
    public function addHeatmap($idSite, $name, $matchPageRules, $sampleLimit = 1000, $sampleRate = 5, $excludedElements = false, $screenshotUrl = false, $breakpointMobile = false, $breakpointTablet = false)
    {
        ...
        return $this->siteHsr->addHeatmap($idSite, $name, $matchPageRules, $sampleLimit, $sampleRate, $excludedElements, $screenshotUrl, $breakpointMobile, $breakpointTablet, $createdDate);
    }
     ...
     * @param int $breakpointTablet   If the device type cannot be detected, we will put any device having a lower width than this value into the tablet category. Useful if your website is responsive.
     */
    public function updateHeatmap($idSite, $idSiteHsr, $name, $matchPageRules, $sampleLimit = 1000, $sampleRate = 5, $excludedElements = false, $screenshotUrl = false, $breakpointMobile = false, $breakpointTablet = false)
    {
        $this->validator->checkWritePermission($idSite);
        ...
        $this->siteHsr->updateHeatmap($idSite, $idSiteHsr, $name, $matchPageRules, $sampleLimit, $sampleRate, $excludedElements, $screenshotUrl, $breakpointMobile, $breakpointTablet, $updatedDate);
    }
     ...
     * @param int $captureKeystrokes  If enabled (default), any text that a user enters into text form elements will be recorded. Password fields will be automatically masked and you can mask other elements with sensitive data using a data-piwik-mask attribute.
     * @return int
     */
    ...
    public function getHeatmaps($idSite)
    {
        ...
        return $this->siteHsr->getHeatmaps($idSite);
    }
    ...
    public function getRecordedSession($idSite, $idSiteHsr, $idLogHsr)
    {
        ...
        $session['idvisitor'] = bin2hex($session['idvisitor']);
        ...
        $session['browser_name'] = \Piwik\Plugins\DevicesDetection\getBrowserName($session['config_browser_name']);
        $session['browser_logo'] = \Piwik\Plugins\DevicesDetection\getBrowserLogo($session['config_browser_name']);
        $session['os_name'] = \Piwik\Plugins\DevicesDetection\getOsFullName($session['config_os']);
        $session['os_logo'] = \Piwik\Plugins\DevicesDetection\getOsLogo($session['config_os']);
        ...
        if (!empty($session['location_country'])) {
            ...
            if (!empty($session['location_region']) && $session['location_region'] != Visit::UNKNOWN_CODE) {
                $session['location_name'] .= ', ' . GeoIp::getRegionNameFromCodes($session['location_country'], $session['location_region']);
            }
