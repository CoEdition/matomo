...
class Aggregator
{
    ...
    public function findRecordings($visitIds)
    {
        ...
        $query = sprintf('SELECT hsrsite.idsitehsr,
                   ...
                   LEFT JOIN %s hsrevent ON hsrevent.idloghsr = hsr.idloghsr and hsrevent.event_type = %s
                   LEFT JOIN %s sitehsr ON hsrsite.idsitehsr = sitehsr.idsitehsr
                   WHERE hsr.idvisit IN ("%s") and sitehsr.record_type = ? and hsrevent.idhsrblob is not null and hsrsite.idsitehsr is not null
                   GROUP BY hsr.idvisit',
            Common::prefixTable('log_hsr'), Common::prefixTable('log_hsr_site'), Common::prefixTable('log_hsr_event'), RequestProcessor::EVENT_TYPE_INITIAL_DOM, Common::prefixTable('site_hsr'), implode('","', $visitIds));

        return Db::fetchAll($query, array(SiteHsrDao::RECORD_TYPE_SESSION));
    }
